 ...
 * @author victo
 */
public class NodoListX {

    private NodoX cabeza;
    private NodoX cola;

    public NodoListX() { // Constructor de la clase NodoListaHorizontal
        this.cabeza = this.cola = null; // Inicializa cabeza y cola como nulos
    }

    public void insertarEnHorizontal(NodoX nodoHorizontal) { // Método para insertar un nodo en la lista horizontal
        ...
        if (cabeza == null) { // Verifica si la cabeza está vacía
            ...
            if (nodoHorizontal.getDato() < cabeza.getDato()) { // Verifica si el nuevo nodo tiene un valor menor que el de la cabeza
                cabeza = insertarInicio(nodoHorizontal); // Inserta el nuevo nodo al inicio de la lista
            } else if (nodoHorizontal.getDato() > cola.getDato()) { // Verifica si el nuevo nodo tiene un valor mayor que el de la cola
    ...
    private NodoX insertarInicio(NodoX nodoHorizontal) { // Método para insertar un nodo al inicio de la lista
    ...
    // nuull -> 2 -> 1 
    private NodoX insertarFinal(NodoX nodoHorizontal) { // Método para insertar un nodo al final de la lista
        ...
        NodoX actual = cabeza; // Inicializa actual como la cabeza
    ...
    private NodoX insertarEnMedio(NodoX nodoHorizontal) { // Método para insertar un nodo en medio de la lista
        // buscamos el nodo después del cual se insertará el nuevo nodo
        NodoX actual = cabeza; // Inicializa actual como la cabeza
        while (actual != null && nodoHorizontal.getDato() > actual.getDato()) { // Itera hasta encontrar el nodo adecuado
            actual = actual.getDerecha(); // Avanza al siguiente nodo
        }
        ...
        NodoX anterior = actual.getIzquierda(); // Obtiene el nodo anterior al nodo actual
    ...
    public void mostrarLista() { // Método para mostrar la lista vertical
        NodoX actual = cabeza; // Crea un nodo auxiliar y lo inicializa con la cabeza
